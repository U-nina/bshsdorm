...
if "initialized" not in st.session_state:
    ...
    # Q&A 초기 샘플 데이터 탑재 (남이 쓴 글 조회 테스트용)
...
# --- 2. 내비게이션 메뉴 구성 (요구사항 8: 로그인 여부와 상관없이 무조건 노출) ---
...
def check_login_permission():
    """메뉴 진입 전 로그인 필수 검사 장치 (요구사항 8 반영)"""
    if not st.session_state.logged_in:
        st.warning("🔒 이 메뉴는 로그인 후 이용하실 수 있는 서비스입니다. 왼쪽 사각 박스에서 먼저 로그인을 진행해 주세요.")
        return False
    return True
...
# 3. 벌점 관리
    ...
    if check_login_permission():
        if st.session_state.user_type == "teacher":
            ...
            # 한시적 퇴사 지도 팝업 표출 조건식 [cite: 9, 10, 11]
            if my_score >= 10:
            ...
            if day == 0:
                ...
                with cols[idx]:
                    ...
                    if current_date_str in academic_calendar:
                        # 중복 코드 제거 및 단일 표기화 (요구사항 3 완벽 반영)
                        st.markdown(
...
# 6. Q&A 게시판
    ...
    # 세션 상태로 서브 메뉴 관리 (글 작성과 목록을 전환할 수 있는 목록 버튼 제공)
    ...
    # 목록 조회 모드 (요구사항 4: 다른 사람이 쓴 글도 리스트로 정상 열람 가능)
...
# 7. 실시간 외박 현황
    ...
    # 조회 날짜 수동 제어판 추가 (요구사항 5 반영 - 기본값 오늘 날짜 설정)
