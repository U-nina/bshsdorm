...
if "initialized" not in st.session_state:
    ...
    # Q&A 초기 샘플 데이터 탑재 (남이 쓴 글 조회 테스트용 - 요구사항 4 반영)
...
def get_dynamic_diet():
    ...
    base_menu = {
        ...
        3: {"day_name": "목요일", "조식": "흰밥, 북어해장국, 감자조림", "중식": "낙지비빔밥, 계란파국, 탕수욕", "석식": "오곡밥, 순두부찌개, 오리훈제볶음"},
    ...
    for i in range(5):
        ...
        dynamic_diet.append({
            ...
            "조식": base_menu[i]["조식"], "중식": base_menu[i]["중식"], "석식": base_menu[i]["석식"]
        })
    return dynamic_diet


# --- 2. 내비게이션 메뉴 구성 (요구사항 8: 로그인 상태 무관하게 상시 오픈) ---
...
# --- 3. 사용자 인증 및 패스워드 통제 마이페이지 ---
st.sidebar.markdown("## 🔑 사용자 인증 센터")
...
if not st.session_state.logged_in:
    ...
    if st.session_state.user_type == "student":
        ...
        st.sidebar.success("👨‍🏫 사감 관리자(교사) 계정 로그인 중")

    # 학생 비밀번호 변경 컴포넌트 활성화 (요구사항 6 반영)
    if st.session_state.user_type == "student":
...
def check_login_permission():
    """로그인 유도 인터셉터 기능 (요구사항 8 반영)"""
    if not st.session_state.logged_in:
        st.warning("🔒 이 메뉴는 로그인 후 이용하실 수 있는 서비스입니다. 왼쪽 사용자 인증 센터에서 먼저 로그인을 진행해 주세요.")
        return False
    return True
...
# 2. 외박 신청
    ...
    if check_login_permission():
        if st.session_state.user_type == "student":
            ...
            if len(my_outings) >= 2:
                st.warning("⚠️ 알림: 외박 신청은 주 2회까지만 허용됩니다. (현재 주 3회 이상 초과 신청 불가)")
            ...
            with st.form("outing_form"):
                ...
                # 버튼 명칭 '외박 신청'으로 변경 (요구사항 2 반영)
                if st.form_submit_button("외박 신청"):
                    # 예외 사유 누락 검사 강화 (요구사항 1 반영)
                    if reason_cat in ["학교 행사", "개인 사정"] and not detail_reason.strip():
                        ...
                        st.success("✅ 외박 신청서가 정상적으로 사감실에 접수되었습니다.")
                        st.rerun()

            st.markdown("---")
            st.subheader("내 외박 신청 내역 및 결과")
...
# 4. 오늘의 식단
    ...
    st.header("🍱 이번 주 기숙사 식단표")
    st.caption("🔗 부산고등학교 급식 안내 연동 정보")
...
# 5. 학사일정 캘린더
    ...
    st.header("📅 2026학년도 주요 학사일정 캘린더")
    months_dict = {"3월": 3, "4월": 4, "5월": 5, "6월": 6, "7월": 7, "8월": 8, "9월": 9, "10월": 10, "11월": 11, "12월": 12}
    selected_month_name = st.selectbox("조회 대상 월 선택", list(months_dict.keys()))
            ...
            if day == 0:
                ...
                with cols[idx]:
                    ...
                    if current_date_str in academic_calendar:
                        # 중복 코드 제거 및 텍스트 박스 단일 표기 처리 (요구사항 3 반영)
                        st.markdown(
...
# 6. Q&A 게시판
    ...
    # 목록 조회 모드 (요구사항 4: 남이 쓴 글 목록도 상시 열람 가능)
                ...
                with st.container(border=True):
                    ...
                    if can_view:
                        ...
                        if post['reply']:
                            ...
                            st.warning("↳ 사감실에서 답변을 건의 검토 중입니다.")
...
# 7. 실시간 외박 현황 (들여쓰기 IndentationError 완전 해결 장치)
    ...
    # ⚠️ 들여쓰기 깊이(Depth) 에러 영역 완벽 교정 완료
