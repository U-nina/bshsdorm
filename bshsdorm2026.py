...
def fetch_neis_diet(target_date_str):
    ...
    params = {
        "KEY": cba565bba5a24574b32236c6e0e895fa,
        "Type": "json",
        "pIndex": 1,
        "pSize": 10,
    ...
    try:
        ...
        if "mealServiceDietInfo" in data:
            ...
            # 주말이나 방학 등 식단 데이터가 서버에 존재하지 않는 사유 예외 대응
            error_msg = "주말 기숙사 홈커밍일 또는 급식 미운영일입니다."
...
# 5. 학사일정 캘린더 (연도/월 선택 박스를 하나로 병합 - 요구사항 1 반영)
elif selected_menu == "📅 학사일정 캘린더":
    st.header("📅 학사일정 월별 스마트 캘린더")

    # 🛠️ 2026년 3월부터 2027년 2월까지 콤보박스 단일 통합 리스트 구성
    calendar_navigation = [
    ...
    # 선택된 텍스트에서 연도(Year)와 월(Month) 파싱 분리
