...
def fetch_neis_diet(target_date_str):
    ...
    params = {
        ...
        "pIndex": 1,
        "pSize": 10,
        "ATPT_OFCDC_SC_CODE": "C10",  # 부산광역시교육청 고유코드
        "SD_SCHUL_CODE": "7150073",  # 부산고등학교 고유코드
    ...
    try:
        ...
        if "mealServiceDietInfo" in data:
            ...
            for row in row_data:
                ...
                # HTML 줄바꿈 태그 변경 및 불필요 알레르기 숫자 정제 규칙 적용
        ...
        return {"조식": "데이터 수신 오류", "중식": "나이스 API 확인 요망", "석식": "사감실 문의"}
...
# 4. 오늘의 식단 (🛠️ 가짜 데이터 대신 나이스 실시간 API 연동 적용 완료)
    ...
    # 각 요일별로 나이스 API를 동적 호출하여 정렬 바인딩
    for i in range(5):
